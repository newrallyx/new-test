...
| 本地正式产物 | `release\旅行轨迹记录与规划工具 0.4.0.exe` |
| SHA-256 | `3C9281ED78CA93181A3DEB22F3B5769D75A46CBB535849BE4554D97D14BFEA39` |

`release*` 目录默认不提交到 Git。GitHub 仓库保存源代码，正式 EXE 需要通过发布渠道获取，或在 Windows 上按本文档自行打包。

当前正式版已经整合照片库、预计行驶时间和预估过路费、完整 ZIP 备份、旧路线缓存兼容、本地数据损坏保护，以及桌面后端访问限制等改进。`0.4.0` 新增自动生成游记功能。
...
- 在“复盘 → 路书浏览”中打开某个旅程，点击“生成游记”，根据已填写的轨迹与路线信息自动生成一篇 Markdown 游记。
- 生成内容按天分节，包含起终点与途经点、里程/预计用时/过路费、实际记录、复盘标签、风景与难度评分、路段备注和照片数量。
- 缺失的数据会被自动省略；可预览渲染效果、重新生成、保存到旅程，或下载为 Markdown 文件。
- 游记保存在旅程数据中，随 ZIP 备份一起导出。
